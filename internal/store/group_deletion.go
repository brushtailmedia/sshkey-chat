...
// PendingAdminKick is one row from the pending_admin_kicks queue —
// an admin-triggered group removal that the CLI has staged for the
// running server to broadcast. Returned by ConsumePendingAdminKicks.
type PendingAdminKick struct {
	ID       int64
	UserID   string
	GroupID  string
	Reason   string
...
// RecordPendingAdminKick queues an admin-triggered group removal so the
// running server can broadcast the leave/group_event for the kicked
// user. Called from the CLI's remove-from-group command after the CLI
// has already mutated group_members directly. The queue exists purely
// for live broadcast delivery; the kick takes effect at the data layer
// regardless of whether the server processes the queue row.
func (s *Store) RecordPendingAdminKick(userID, groupID, reason string) error {
	if reason == "" {
		reason = "admin"
	}
	_, err := s.dataDB.Exec(
		`INSERT INTO pending_admin_kicks (user_id, group_id, reason, queued_at) VALUES (?, ?, ?, ?)`,
		userID, groupID, reason, time.Now().Unix(),
	)
	return err
}

// ConsumePendingAdminKicks reads every pending kick row, deletes them
// all atomically, and returns the consumed list to the caller. The
// caller is then responsible for performing the broadcasts.
//
// Atomic semantics: a transaction wraps the SELECT and DELETE so a
// concurrent invocation can't double-process a kick. The transaction
// is short-lived (typically zero or a handful of rows) so contention
// is negligible.
func (s *Store) ConsumePendingAdminKicks() ([]PendingAdminKick, error) {
	...
	rows, err := tx.Query(
		`SELECT id, user_id, group_id, reason, queued_at FROM pending_admin_kicks ORDER BY id`,
	)
	if err != nil {
		return nil, err
	}

	var kicks []PendingAdminKick
	for rows.Next() {
		var k PendingAdminKick
		if err := rows.Scan(&k.ID, &k.UserID, &k.GroupID, &k.Reason, &k.QueuedAt); err != nil {
			rows.Close()
			return nil, err
		}
	...
	if len(kicks) > 0 {
		if _, err := tx.Exec(`DELETE FROM pending_admin_kicks`); err != nil {
			return nil, err
		}
	}
